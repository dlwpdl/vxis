...
type: concept
status: active
when_to_read: 5차원 가중치 / 벡터 ID 매핑 / 새 skill 추가 시 scoring 연결 / 등급 기준 / FP Bayesian smoothing / skill alias 가 VC 깨는 함정
updated: 2026-04-23
sources:
...
- **Skill alias gotcha (Q10, commit `ce5a2f8`)**: scan_loop iter ≥ 25 sweep 은 큐 alias (`test_dylib_hijack__sweep25`) 를 주입한다. `run()` 이 `_skills_completed` (alias 포함) 를 리턴하면 `_DESKTOP_SKILL_TO_VECTORS` 의 real-name 키와 매치 안 됨 → VC=0. 해결: `_real_skills_completed` (실명만) 를 별도 트래킹하고 리턴 dict 에 그것만 노출. Brain-direct `run_skill` 도 같은 set 에 add 해 manual pick 도 크레딧.
