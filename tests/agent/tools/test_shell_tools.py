...
from vxis.agent.tools.shell_tools import (
    ShellExecTool,
    _reset_for_tests,
    _sandbox_network_and_caps,
    SANDBOX_IMAGE,
    resolve_sandbox_runtime,
)


def test_sandbox_network_host_and_net_raw_when_ghost_off():
    network, caps = _sandbox_network_and_caps()
    assert network == ["--network", "host"]
    assert caps == ["--cap-add", "NET_RAW"]


def test_sandbox_isolated_bridge_no_net_raw_when_ghost_active(monkeypatch):
    ...
    assert network == ["--network", "bridge"]
    assert caps == []


def test_sandbox_keeps_host_network_when_direct_egress_opt_in(monkeypatch):
    from vxis.ghost.layer import ghost_layer
...
async def test_shell_exec_tool_image_not_built():
    ...
    with (
        patch("vxis.agent.tools.shell_tools._docker_available", return_value=True),
        patch("vxis.agent.tools.shell_tools._image_exists", side_effect=fake_image_exists),
        patch(
            "vxis.agent.tools.shell_tools._container_running", side_effect=fake_container_running
        ),
    ):
        tool = ShellExecTool()
        result = await tool.run(command="echo hi")
...
async def test_ensure_sandbox_running_uses_per_scan_container_and_workspace(tmp_path):
    ...
    assert ("--log-opt", "max-size=50m") == run_call[
        run_call.index("--log-opt") : run_call.index("--log-opt") + 2
    ]


@pytest.mark.asyncio
async def test_ensure_sandbox_running_applies_opt_in_resource_limits(tmp_path, monkeypatch):
    from vxis.agent.tools import shell_tools

    monkeypatch.setenv("VXIS_SANDBOX_MEM_LIMIT", "768m")
    monkeypatch.setenv("VXIS_SANDBOX_SHM_SIZE", "128m")
    monkeypatch.setenv("VXIS_SANDBOX_CPUS", "1.5")
    monkeypatch.setenv("VXIS_SANDBOX_PIDS_LIMIT", "256")
    calls: list[tuple[str, ...]] = []
    ...
    with (
        ...
        ok, _msg = await shell_tools._ensure_sandbox_running(
            sandbox_key="limited",
            workspace_host=str(tmp_path),
        )
    ...
    run_call = next(call for call in calls if call[:1] == ("run",))
    assert ok is True
    assert ("--memory", "768m") == run_call[
        run_call.index("--memory") : run_call.index("--memory") + 2
    ]
    assert ("--shm-size", "128m") == run_call[
        run_call.index("--shm-size") : run_call.index("--shm-size") + 2
    ]
    assert ("--cpus", "1.5") == run_call[run_call.index("--cpus") : run_call.index("--cpus") + 2]
    assert ("--pids-limit", "256") == run_call[
        run_call.index("--pids-limit") : run_call.index("--pids-limit") + 2
    ]
...
async def test_shell_exec_tool_runs_command_via_docker_exec_when_sandbox_ready():
    ...
    with (
        patch("vxis.agent.tools.shell_tools._ensure_sandbox_running", side_effect=fake_ensure),
        patch(
            "vxis.agent.tools.shell_tools._execute_via_tool_server", AsyncMock(return_value=None)
        ),
...
async def test_shell_exec_tool_uses_tool_server_when_available():
    ...
    with (
        patch("vxis.agent.tools.shell_tools._ensure_sandbox_running", side_effect=fake_ensure),
        patch(
            "vxis.agent.tools.shell_tools._execute_via_tool_server",
            AsyncMock(return_value=server_result),
        ),
