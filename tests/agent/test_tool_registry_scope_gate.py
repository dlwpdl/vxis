...
class _OfflineTool:
    ...
    def __init__(self): self.ran = False
    async def run(self, **kwargs): self.ran = True; return ToolResult(ok=True, summary="reported")
...
async def test_offline_tool_bypasses_scope_gate():
    ...
    reg = ToolRegistry(); tool = _OfflineTool(); reg.register(tool)
    set_active_scope(_scope(["app.acme.com"]))
...
async def test_destructive_approval_at_dispatch():
    ...
    tool2 = _Tool(); reg2 = ToolRegistry(); reg2.register(tool2)
