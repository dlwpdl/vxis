...
_CORE_SKILL_CARDS: tuple[SkillCard, ...] = (
    ...
    SkillCard(
        ...
        surface="web",
        families=("api", "auth", "idor", "rate_limit", "bopla", "bfla"),
        ...
        method="Check method tampering, mass assignment, parameter pollution, object/property auth, function auth, and rate-limit behavior with controls.",
        validation="Require before/after response evidence and an object, property, function, or state boundary impact.",
    ...
    SkillCard(
        name="test_auth_deep",
        surface="web",
        families=("auth", "jwt", "session", "oauth", "saml"),
        roles=("director", "exploit_worker", "post_exploit_worker"),
        triggers=("jwt", "session", "reset", "oauth", "oidc", "saml", "jku", "kid", "role", "claim", "cookie"),
        objective="Attack token/session design after basic auth surface is mapped.",
        method="Check JWT alg/kid/jku/claim tampering, OAuth/OIDC/SAML state or replay confusion, session fixation, reset poisoning, and role transition controls.",
    ...
    SkillCard(
        name="test_misconfig",
        surface="web",
        families=("misconfig", "cors", "headers", "debug", "cache"),
        roles=("director", "recon_worker", "review_worker"),
        triggers=("cors", "header", "debug", "error", "misconfig", "verbose", "swagger", "actuator", "cache", "proxy"),
        objective="Find security-impacting misconfigurations, not cosmetic header gaps.",
        method="Check CORS, debug endpoints, verbose errors, exposed docs, cache/proxy boundary leaks, and risky headers in context.",
