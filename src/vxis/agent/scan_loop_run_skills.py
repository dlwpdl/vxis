        ...
        *,
        ...
        if "run_skill" in self.registry.list_tools():
            for skill_name, trigger_iter, extra_params in skill_sequence:
                if skill_name not in skills_completed and self.state.iteration >= trigger_iter:
                    ...
                    try:
                        ...
                        sr = await self.registry.dispatch(
                            "run_skill",
                            {
                                "skill": _real_skill,
                        ...
                        if sr.ok:
                                    ...
                                    if sev in ("critical", "high"):
                                        ...
                                        await self._dispatch_report_finding_checked(
                                            self._build_report_finding_args(
                                                ...
                                                or f"Sensitive file {exposed['path']} is externally accessible.",
                                ...
                                for finding in sr.data.get("findings") or []:
                                    ...
                                    args = {
                                        ...
                                        "evidence": finding.get(
                                            "response_preview", finding.get("evidence", "")
                                        )[:500],
                                    }
                                    args.update(
                                        self._build_report_finding_args(
                            ...
                            if (
                                _real_skill in ("test_injection", "test_xss", "test_ssrf")
                                and sr.data
                            ):
                                ...
                                if not sr.data.get("vulnerable") and _cur_round < 3:
                                    ...
                                    if _url:
                                        ...
                                        _next = _cur_round + 1
                                        _alias_r = f"{_real_skill}__round{_next}_iter{self.state.iteration}"
                                        queue_skill(
                                            _real_skill,
                                            self.state.iteration + 2,
                                            {
                            ...
                            if _real_skill == "test_ssrf" and sr.data:
                                for finding in sr.data.get("findings") or []:
                                    ...
                                    matched_signal = str(
                                        control.get("matched_signal")
                                        or finding.get("type", "internal_response")
                                    )
                                    callback_summary = (
                                        response_preview[:500]
                                        or str(finding.get("evidence", ""))[:500]
                                    )
                            ...
                            if _real_skill == "test_misconfig" and sr.data:
                                for finding in (sr.data.get("findings") or [])[:5]:
                                    ...
                                    mis_desc = finding.get("description", finding.get("type", ""))[
                                        :200
                                    ]
                                    mis_evidence = finding.get(
                            ...
                            if _real_skill == "test_api_security" and sr.data:
                                for finding in (sr.data.get("findings") or [])[:5]:
                                    ...
                                    api_desc = finding.get(
                                        "description", finding.get("payload", "")
                                    )[:200]
                            ...
                            if _real_skill == "test_infra" and sr.data:
                                for finding in sr.data.get("findings") or []:
                                    ...
                                    infra_poc = self._build_simple_http_poc(
                                        ...
                                        response_preview=finding.get(
                                            "response_preview", infra_evidence
                                        )[:1200],
                                    )
                                    await self._dispatch_report_finding_checked(
                                        self._build_report_finding_args(
                                    ...
                                    await self._dispatch_report_finding_checked(
                                        {
                                            "title": finding.get(
                                                "title",
                                                f"Desktop finding: {finding.get('vector', '?')}",
                                            ),
                                            "severity": finding.get("severity", "medium"),
                                            "finding_type": finding.get(
                                                "vector", "desktop_misconfiguration"
                                            ),
