...
class BluetoothDeepAgent(BaseAgent):
    ...
    async def run(self, context: AgentContext) -> AgentResult:
        ...
        tools_available = [t for t, ok in [
            ("hcitool", has_hcitool), ("bluetoothctl", has_bluetoothctl),
            ("spooftooph", has_spooftooph), ("redfang", has_redfang),
    ...
    async def _run_gatt_discovery(self, bd_addr: str) -> list[str]:
        ...
        proc = await asyncio.create_subprocess_exec(
            "gatttool", "-b", bd_addr, "--characteristics",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.DEVNULL,
        )
        try:
