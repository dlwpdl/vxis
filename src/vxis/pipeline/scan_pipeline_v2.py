...
def _compute_vxis_score(ctx: Any) -> tuple[float, str]:
    ...
    try:
        ...
        # Map finding types → registry vector IDs (vxis.scoring.vectors.WEB_VECTORS).
        # All IDs here must exist in the registry; non-matching IDs score 0 on
        # the vector-coverage dimension because ScoringEngine intersects with
        # valid_ids before counting.
        _type_to_vector: dict[str, str] = {
            ...
            "network_appliance_rce":      "WEB-INFRA-006",
            # ── Supply Chain ──
