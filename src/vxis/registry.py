...
WEB_PHASES: list[PhaseInfo] = [
    ...
    PhaseInfo(
        3,
        "Hypothesis Engine — Pattern Matching",
        "가설 엔진 — 패턴 매칭",
        "intelligence",
        "_phase3_hypothesis",
        depends_on=(4,),
    ...
    PhaseInfo(
        2,
        "63 Autonomous Agents — Brain-Directed Dispatch",
        "63개 자율 에이전트 — Brain 지휘",
        "intelligence",
        "_phase2_agents",
        depends_on=(4, 3),
        parallel_group=4,
    ...
    PhaseInfo(
        5,
        "Special Agents (IoT/VoIP/Web3)",
        "특수 에이전트 (IoT/VoIP/Web3)",
        "exploitation",
        "_phase5_special",
        depends_on=(2,),
        parallel_group=5,
    ...
    PhaseInfo(
        7,
        "Hardware Agents (DMA/SS7/Cold Boot)",
        "하드웨어 에이전트",
        "exploitation",
        "_phase7_hardware",
        depends_on=(2,),
    ...
    PhaseInfo(
        11,
        "Chain Mutation — Alternative Attack Paths",
        "체인 변이 — 대체 공격 경로",
        "chain",
        "_phase11_mutation",
        depends_on=(8,),
    ...
    PhaseInfo(
        12,
        "Self-Evolving Agent — Coverage Gap Analysis",
        "자가 진화 — 커버리지 갭 분석",
        "learning",
        "_phase12_evolution",
        depends_on=(11,),
    ...
    PhaseInfo(
        6,
        "Report Generation — NCC Group Style",
        "리포트 생성 — NCC Group 스타일",
        "report",
        "_phase6_report",
        depends_on=(12, 18),
        parallel_group=9,
...
EXTERNAL_PHASES: list[PhaseInfo] = [
    PhaseInfo(
        9, "CVE Watch — Component Vulnerability Matching", "CVE 감시", "external", "cve-watch.yml"
    ),
    PhaseInfo(
        14,
        "Temporal Vulnerability Forecast",
        "시간 기반 취약점 예측",
        "external",
        "domain-intel.yml",
    ),
    PhaseInfo(
        16,
        "Industry Intelligence — Sector Risk Heatmap",
        "산업 인텔리전스",
        "external",
        "domain-intel.yml",
    ),
...
FUTURE_PHASES: list[PhaseInfo] = [
    PhaseInfo(
        10,
        "Red vs Blue — Defense Rule Generation",
        "레드 vs 블루 — 방어 규칙 생성",
        "future",
        "_phase10_red_vs_blue",
    ),
