...
- [x] **Single-source flagship + Settings refresh** (`04f1ac9`): `model_registry.flagship(provider)` is now the one place the default model per provider lives; `agent/brain.py` failover chain pulls the Anthropic flagship from it (opus-4-6 → opus-4-8, incl. [1m]) — verified the chain leads with opus-4-8. `model_catalog.clear_cache()` + TUI **Settings** is now a real submenu ("🔄 모델 소스 새로고침" clears cache + live re-fetch + reports source/count; "🔑 환경변수 안내"). **Intentionally NOT done:** routing brain.py's Together cost-tier fallbacks (gpt-oss/Qwen variants) or `hybrid_config.py` through the live catalog — those are role/cost-tuned with curated fallbacks the registry lacks; wholesale routing would drop fallbacks / change role defaults. hybrid_config was already current (sonnet-4-6 / gemini-3.1-pro / Kimi).
